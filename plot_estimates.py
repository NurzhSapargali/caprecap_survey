...
schnab = pd.read_csv(DIR.format("schnab_3.0.csv"), header=None)
...
estis = pd.read_csv(DIR.format("estis_3.0.csv"), header=None)
...
plt.close(fig)
jacks = pd.read_csv(DIR.format("jks_3.0.csv"), header=None)
...
sns.boxplot(x="log alpha_hat", y="T", orient="h",
            ...
            ax=ax)
ax.axvline(x=np.log(3.0))
fig.savefig(FIGS.format("alpha_hat.pdf"), bbox_inches="tight")
plt.close(fig)
nu_trace = data = pd.read_csv(DIR.format("Nu_trace_3.0.csv"), header=None)
...
for T in [2, 4, 10]:
    ...
    plt.close(fig)
alpha_trace = pd.read_csv(DIR.format("alpha_trace_3.0.csv"), header=None)
