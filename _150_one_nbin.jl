...
function log_likelihood(logit_w, log_a, log_Nu, f)
    No = sum(values(f))
    N = exp(log_Nu) + No
    a = exp(log_a)
...
function gradient_logit_w(logit_w, log_a, log_Nu, f)
    No = sum(values(f))
    N = exp(log_Nu) + No
    a = exp(log_a)
    #w = 1.0 / (1.0 + exp(-logit_w))
...
function fit_oi_nbin_trunc(
    ...
    function objective(x::Vector, grad::Vector)
        if length(grad) > 0
            grad[1] = -gradient_logit_w(x[1], x[2], x[3], f)
...
function fit_oi_geom_trunc(
    ...
    function objective(x::Vector, grad::Vector)
        if length(grad) > 0
            grad[1] = -gradient_logit_w(x[1], log(1.0), x[2], f)
