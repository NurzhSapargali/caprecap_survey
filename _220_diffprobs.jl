...
for pop in N
    for alpha in ALPHAS
        metafile = DATA_FOLDER * "alpha_$(alpha)/metadata_$(alpha)_$(pop).csv"
        write_row(metafile, vcat(["N", "T", "alpha"], ["n_$i" for i in 1:T]))
        b = alpha * (pop - 1.0)
        d = truncated(Beta(alpha, b), upper = 1.0 / maximum(n))
        p = rand(d, pop)
        write_row(metafile, vcat([pop, T, alpha], n))
        write_row(metafile, vcat(["p"], p))
        for trial in 1:TRIALS
            println("Generating samples for $n")
            samples = [sample(1:pop, pweights(n[i] * p), n[i], replace = false) for i in 1:T]
