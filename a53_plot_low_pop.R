...
for (het in ALPHAS){
  ...
  prep <- read.csv(
    paste0(RESULTS_FOLDER, "estimates_", het_str, "_low_pop.csv")
  ) %>%
    preprocess()
  ...
  ggarrange(plotlist = comp_ps, ncol = 2, nrow = 2)
  ggsave(
    paste0(OUTPUT_FOLDER, "estimates_", het_str, "_low_pop.pdf"),
    width = 210,
    height = 198,
    units = "mm"
  )
...
ggsave(
  paste0(OUTPUT_FOLDER, "relative_bias_low_pop.pdf"),
  width = 210,
  height = 99,
  units = "mm"
)
